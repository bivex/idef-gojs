...
## 🏛️ 10. Стандарт IDEF12 — Organization Modeling (KBSI / US Air Force / IICE)

**IDEF12** — метод **организационного моделирования** (Organization Modeling), предназначенный для формализации и анализа организационных структур предприятий, штатных позиций, ролевых матриц ответственности (RACI) и квалификационных допусков персонала опасных и наукоемких производств.

> «IDEF12 provides a method for representing organizational structures, authority lines, positions, RACI roles, and competency constraints across enterprise boundaries.» — KBSI / US Air Force Technical Vision

### Основные концепции IDEF12:

| Концепция | Описание | Пример (ОАО «Металл-Авиа») |
|-----------|----------|---------------------------|
| **Организационная единица (OrgUnit)** | Подразделение предприятия (DIVISION, DEPARTMENT, WORKSHOP, LABORATORY, BRIGADE, EXTERNAL) со штатной численностью и локацией | OU-DIR-GEN (Генеральная дирекция), OU-SHOP-03 (Цех №3 жаропрочного литья) |
| **Штатная должность (Position)** | Должностная позиция в штатном расписании (EXECUTIVE, MANAGEMENT, ENGINEER, OPERATOR) с грейдом и списком обязанностей | POS-CHIEF-MET (Главный металлург завода, Грейд 16), POS-OP-VACUUM (Оператор печи, 6 разряд) |
| **Организационная роль (OrgRole)** | Роль в матрице RACI (ACCOUNTABLE, RESPONSIBLE, CONSULTED, INFORMED, AUDITOR) с областью ответственности | ROLE-PROC-OWNER (Владелец процесса термообработки), ROLE-BATCH-ACCEPT (Аудитор приёмки) |
| **Компетенция / Допуск (Competency)** | Требование к аттестации, допуску по промбезопасности или сертификату квалификации | COMP-RTN-B2 (Аттестация Ростехнадзора Б.2), COMP-NDT-3 (ВИК/УЗК III уровня) |

### Типы связей IDEF12:

| Тип связи | Семантика | Визуализация |
|-----------|-----------|--------------|
| **SUBORDINATE_TO** | Иерархическое административное подчинение (снизу вверх) | 🔵 Тёмно-синяя сплошная |
| **ASSIGNED_TO** | Штатная расстановка (назначение должности в подразделение) | 🟢 Зелёная сплошная |
| **FUNCTIONAL_REPORTS** | Функциональное / матричное руководство и контроль | 🟣 Фиолетовая штриховая |
| **PLAYS_ROLE** | Закрепление роли матрицы RACI за должностью | 🔴 Розовая пунктирная |
| **REQUIRES_COMPETENCY** | Требование обязательного допуска / компетенции к должности | 🟡 Янтарная пунктирная |
| **COLLABORATES_WITH** | Межфункциональное горизонтальное взаимодействие подразделений | ⚪ Серая штриховая |

### Правила валидации IDEF12:
1. **Ацикличность административного подчинения**: граф связей `SUBORDINATE_TO` обязан образовывать строгое дерево или ациклический граф (DAG) → ошибка `IDEF12_SUBORDINATION_CYCLE`.
2. **Должности без подразделения (Orphan Position)**: штатная позиция без связи `ASSIGNED_TO` → предупреждение `IDEF12_ORPHAN_POSITION`.
3. **Критическая роль без исполнителя**: роли утверждения (`ACCOUNTABLE`) и контроля (`AUDITOR`) обязаны быть назначены должностям через `PLAYS_ROLE` → предупреждение `IDEF12_ROLE_WITHOUT_ASSIGNEE`.
4. **Уникальность кодов**: запрет дубликатов кодов OU-xxx и POS-xxx → ошибка `IDEF12_DUPLICATE_UNIT_CODE` / `IDEF12_DUPLICATE_POSITION_CODE`.
5. **Целостность связей**: обнаружение разорванных ссылок → ошибка `IDEF12_DANGLING_LINK_SOURCE/TARGET`.
