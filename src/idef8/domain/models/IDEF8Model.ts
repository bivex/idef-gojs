import { IDEF8Diagram, IDEF8DiagramProps } from './IDEF8Diagram';

export interface IDEF8ModelProps {
  id: string;
  name: string;
  version?: string;
  diagrams?: IDEF8DiagramProps[];
  activeDiagramId?: string;
}

export class IDEF8Model {
  public readonly id: string;
  private _name: string;
  public readonly version: string;
  private _diagrams: Map<string, IDEF8Diagram>;
  private _activeDiagramId: string;

  constructor(props: IDEF8ModelProps) {
    ...
    if (props.diagrams && props.diagrams.length > 0) {
      props.diagrams.forEach((d) => this._diagrams.set(d.id, new IDEF8Diagram(d)));
      this._activeDiagramId = props.activeDiagramId || props.diagrams[0].id;
    } else {
      const defaultDiagram = new IDEF8Diagram({
        id: 'diag-hmi-root',
        name: 'HMI Flow Diagram',
        description: 'Корневая схема диалога и интерфейса IDEF8',
      });
  ...
  public get diagrams(): ReadonlyArray<IDEF8Diagram> {
    return Array.from(this._diagrams.values());
  }

  public get activeDiagram(): IDEF8Diagram {
    const d = this._diagrams.get(this._activeDiagramId);
    if (!d) {
      const first = this._diagrams.values().next().value;
      if (!first) throw new Error('No diagram found in IDEF8 model.');
  ...
  public addDiagram(diagram: IDEF8Diagram): void {
  ...
  public removeDiagram(diagramId: string): void {
    if (this._diagrams.size <= 1) {
      throw new Error('Cannot remove the last remaining diagram in IDEF8 model.');
    }
  ...
  public static fromJSON(json: any): IDEF8Model {
    if (!json || !json.id || !json.name) {
      throw new Error('Invalid JSON format for IDEF8Model.');
    }
    return new IDEF8Model({
      id: json.id,
      name: json.name,
