import { IDEF9Diagram, IDEF9DiagramProps } from './IDEF9Diagram';

export interface IDEF9ModelProps {
  id: string;
  name: string;
  version?: string;
  diagrams?: IDEF9DiagramProps[];
  activeDiagramId?: string;
}

export class IDEF9Model {
  public readonly id: string;
  private _name: string;
  public readonly version: string;
  private _diagrams: Map<string, IDEF9Diagram>;
  private _activeDiagramId: string;

  constructor(props: IDEF9ModelProps) {
    ...
    if (props.diagrams && props.diagrams.length > 0) {
      props.diagrams.forEach((d) => this._diagrams.set(d.id, new IDEF9Diagram(d)));
      this._activeDiagramId = props.activeDiagramId || props.diagrams[0].id;
    } else {
      const defaultDiagram = new IDEF9Diagram({
        id: 'diag-constraints-root',
        name: 'Enterprise Business Constraints',
        description: 'Корневая схема бизнес-ограничений и регламентов IDEF9',
      });
  ...
  public get diagrams(): ReadonlyArray<IDEF9Diagram> {
    return Array.from(this._diagrams.values());
  }

  public get activeDiagram(): IDEF9Diagram {
    const d = this._diagrams.get(this._activeDiagramId);
    if (!d) {
      const first = this._diagrams.values().next().value;
      if (!first) throw new Error('No diagram found in IDEF9 model.');
  ...
  public addDiagram(diagram: IDEF9Diagram): void {
  ...
  public removeDiagram(diagramId: string): void {
    if (this._diagrams.size <= 1) {
      throw new Error('Cannot remove the last remaining diagram in IDEF9 model.');
    }
  ...
  public static fromJSON(json: any): IDEF9Model {
    if (!json || !json.id || !json.name) {
      throw new Error('Invalid JSON format for IDEF9Model.');
    }
    return new IDEF9Model({
      id: json.id,
      name: json.name,
